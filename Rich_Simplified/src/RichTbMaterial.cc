...
//Sio2 
 ...
 SiO2GasWinQuartz->AddElement(elO,natoms=2);
  ...
  G4double* GasWinQuartzAbsorpLength=new G4double[NumPhotWaveLengthBins];
  G4double* GasWinQuartzRindex = new G4double[NumPhotWaveLengthBins];
  G4double* GasWinQuartzProdCkvRndx = new G4double[CkvProdNumPhotWaveLengthBins];
  

 for (G4int ibin=0; ibin<NumPhotWaveLengthBins; ibin++){
    GasWinQuartzAbsorpLength[ibin]=1.E32  * CLHEP::mm;
  ...
  for (G4int ibina=IbMinForCkv; ibina <IbMaxForCkv ; ibina++){
    ...
    GasWinQuartzProdCkvRndx[ibinp]= GasWinQuartzRindex[ibina];
  ...
  G4MaterialPropertiesTable* GasWinQuartzMPT = 
                            new G4MaterialPropertiesTable();


  GasWinQuartzMPT->AddProperty("ABSLENGTH",PhotonMomentum,
                GasWinQuartzAbsorpLength,NumPhotWaveLengthBins);


  GasWinQuartzMPT->AddProperty("RINDEX", PhotonMomentum, 
                     GasWinQuartzRindex,NumPhotWaveLengthBins);
  GasWinQuartzMPT->AddProperty("CKVRNDX", PhotonMomentumCkvProd, 
                     GasWinQuartzProdCkvRndx,CkvProdNumPhotWaveLengthBins);
...
//Aluminium
  ...
  G4Material* CarbonMaterial = 
   ...
   // 
  density = 1.12 * CLHEP::g / CLHEP::cm3;
  G4Material* CarbonFibreResin = 
    new G4Material (name="CarbonFibreResin", density,numel=4 );
    CarbonFibreResin->AddElement(elC, natoms=36);
    CarbonFibreResin->AddElement(elH, natoms=4);
    CarbonFibreResin->AddElement(elO, natoms=10);
    CarbonFibreResin->AddElement(elN, natoms=1); 
          
  // carbon fibre
  density = 0.167 * CLHEP::g / CLHEP::cm3;
  G4Material* CarbonFibreMaterial = 
      new G4Material (name="CarbonFibreMaterial",
		      density,numel=2);
  CarbonFibreMaterial->AddMaterial(CarbonMaterial, fractionmass=0.70 );
  CarbonFibreMaterial->AddMaterial( CarbonFibreResin,fractionmass=0.30 );
  RichTbCarbonFibreMaterial  = CarbonFibreMaterial;
...
// Kovar
  ...
  // FrontRing
  density=2.7 * CLHEP::g / CLHEP::cm3;
  G4Material* FrontRing =new G4Material(PMTFrontRingMaterialName,density,numel=1);
  FrontRing->AddElement(elAL,natoms=1);
   
  PMTFrontRingMaterial=FrontRing;
 
   // Pmt Module Back Pl
  density=2.7 * CLHEP::g / CLHEP::cm3;
  G4Material* aBackPlMat =new G4Material( PMTModuleBackPlMaterialName,density,numel=1);
  aBackPlMat->AddElement(elAL,natoms=1);
   
  PMTModuleBackPlMaterial  = aBackPlMat ; 
...
// crystal material
 ...
 CrystalMat->AddElement(elO,natoms=2);
  ...
  G4cout << "Now n bin " << CkvProdNumPhotWaveLengthBins << G4endl;
