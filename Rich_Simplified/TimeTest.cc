...
int main( int argc, char** argv, const char* argforced )
{
    ...
    for(int i=0; i<5; i++){
        g4_time[i] = rich1.beamOn(events[i]);
    }
    for(int i=0; i<5; i++){
        std::cout << events[i] << "\t" << g4_time[i] << "\t";
    }
    std::cout << std::endl;
    rich1.Finalize();
#define WITH_OPTIKCKS
    RichTbSimH rich2( argc, argv, argforced );
    double total_time[] = {0,0,0,0,0};
    for(int i=0; i<5; i++){
        total_time[i] = rich2.beamOn(events[i]);
    }
    for(int i=0; i<5; i++){
        std::cout << events[i] << "\t" << total_time[i]-g4_time[i] << "\t";
    }
    std::cout << std::endl;
