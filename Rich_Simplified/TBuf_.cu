...
unsigned TBuf::downloadSelection(const char* name, NPY<float>* selection, unsigned hitmask, bool verbose) const 
{
    ...
    //test
    std::ofstream ofile;
    ofile.open("All_data.txt"); 
    unsigned long long ii = 0;
    // copy a device vector into an STL vector
    std::vector<T> stl_vector(numItems);
    thrust::copy( ptr, ptr+numItems, stl_vector.begin() );
    for ( ii=0; ii<numItems; ii++ ) {
        std::cout << "Output item " << ii << " " << stl_vector[ii] << std::endl;
        ofile<< stl_vector[ii].q0 << std::endl;
    }
    if(verbose)
    std::cout 
        << "TBuf::downloadSelection"
